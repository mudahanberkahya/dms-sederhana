...
def stamp_pdf(input_pdf_path, output_pdf_path, signature_image_path, keyword, position_hint='Above', offset_x=0, offset_y=0, delegate_name=""):
    ...
    Default placement: ABOVE the keyword (30-45px higher than the keyword's bounding box).
    The signature is centered horizontally on the keyword.
    """
    try:
        ...
        if not found:
            print(f"Warning: Keyword '{keyword}' not found in any page. Stamping at default bottom-center location.", file=sys.stderr)
